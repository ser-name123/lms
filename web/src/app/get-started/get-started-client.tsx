...
export default function GetStartedPage({
  ...
  useEffect(() => {
    ...
    const tz = initialTimeZone || detectTimeZone();
    setTimeZone(tz);

    let guess = null;
    if (initialCountryCode) {
      const match = COUNTRIES.find((c) => c.iso2 === initialCountryCode);
      if (match) guess = match;
    }
    if (!guess) {
      guess = detectCountry();
    }
    ...
    if (guess) {
      ...
      if (!initialTimeZone && COUNTRY_TIMEZONES[guess.name]) {
