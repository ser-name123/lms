...
export default function TeacherProfile() {
  ...
  const handleCropAvatar = async (croppedBase64: string) => {
    ...
    setUploading(true);
    try {
      await updateTeacherProfile({ avatarUrl: croppedBase64 });
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (!firstName.trim() || !lastName.trim()) {
      Swal.fire("Names Required", "First name and last name cannot be empty.", "warning");
      return;
    }

    if (password) {
      if (password.length < 8) {
        Swal.fire("Weak Password", "New password must be at least 8 characters long.", "warning");
        return;
      }
      if (password !== confirmPassword) {
        Swal.fire("Mismatched Passwords", "Confirm password does not match.", "warning");
        return;
      }
    }
    ...
    try {
      ...
      Swal.fire("Failed", "Could not update profile information.", "error");
    } finally {
      setSaving(false);
    }
  };
            ...
            <Card className="border border-hairline bg-surface rounded-3xl p-6 text-center space-y-5 hover:shadow-md transition">
              ...
              <div>
                ...
                <p className="text-[10px] text-ink-3 font-extrabold uppercase mt-1">
                  Teacher Code: {profile.teacherCode || "—"}
                </p>
              </div>
              ...
              <div className="border-t border-hairline pt-4 space-y-3 text-left text-xs font-semibold text-ink-2">
                ...
                <p className="flex items-center gap-2.5">
                  <BookOpen className="size-4.5 text-ink-3 shrink-0" />
                  <span>Assigned Subject: {profile.course?.title || "None"}</span>
                </p>
                <p className="flex items-center gap-2.5">
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card className="border border-hairline bg-surface rounded-3xl p-6">
              <form onSubmit={handleFormSubmit} className="space-y-5">
                ...
                <div>
                  <label className="block text-[10px] font-extrabold text-ink-3 uppercase mb-1.5 tracking-wider">
                    Bio Settings (About Yourself)
                  </label>
                  <textarea
                    rows={4}
                    value={bio}
                    onChange={(e) => setBio(e.target.value)}
                    placeholder="Describe your credentials and years of experience teaching..."
                    className="w-full rounded-xl border border-hairline bg-surface p-3.5 text-xs text-ink focus:outline-none focus:ring-2 focus:ring-accent"
                  />
                </div>
